<?php
    ...
    try {
        ...
        if(isset($_POST['btn_confirm'])){
            ...
            if (!empty($_POST['item'])&& $_POST['buyspot'] !== "▼売場を選択してください" && !empty($_POST['buyname']) && !empty($_POST['date']) && !empty($_POST['price'])) { 
            ...
            //form登録
            $sql = "INSERT INTO items (item,buyspot,buyname,price,date) VALUES (?,?,?,?,?)";
            $stmt = $dbh -> query("SET NAMES utf8;");
            $stmt = $dbh ->prepare($sql);
            $stmt->bindValue(1, $_SESSION['item'], PDO::PARAM_STR);
            $stmt->bindValue(2, $_SESSION['buyspot'], PDO::PARAM_STR);
            $stmt->bindValue(3, $_SESSION['buyname'], PDO::PARAM_STR);
            $stmt->bindValue(4, $_SESSION['price'], PDO::PARAM_INT);
            $stmt->bindValue(5, date("Y-m-d",strtotime($_SESSION['date'])));
...
label {
    ...
    <body>
        ...
        <?php if( $page_flag === 1 ): ?>
            <h1>エネルギッシュな缶詰-TOM版確認フォーム-</h1>
            <form method="post" action="newitem_tom.php">
        ...
        <h1>エネルギッシュな缶詰(β)-TOM版登録完了フォーム-</h1>
            <p>登録が完了しました</p>
            <p><a href="enecan.php">TOPページへ戻る</a> | <a href="newitem_tom.php">続けて登録する</a></p>
        <?php else: ?>
        <h1>エネルギッシュな缶詰-TOM版登録フォーム-</h1>
                <form method="POST" action="newitem_tom.php">
                     <!--item -->
                    <div class="form-group row">
                        <label for="item" class="col-sm-2 col-form-label">アイテム名</label>
